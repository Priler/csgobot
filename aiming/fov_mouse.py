...
class FOVMouseMovement:
    ...
    def __init__(
        ...
        # Pre-calculate focal lengths (the key to correct conversion)
        # focal_length = (half_screen) / tan(half_fov)
        self._focal_x = (screen.width / 2) / tan(radians(fov.horizontal / 2))
        self._focal_y = (screen.height / 2) / tan(radians(fov.vertical / 2))

        # Pre-calculate pixels per degree for mouse movement
    ...
    def screen_to_angle(self, target_x: float, target_y: float) -> Tuple[float, float]:
        ...
        """
        # Get pixel offset from screen center (where crosshair is)
        center_x, center_y = self.screen.center
        offset_x = target_x - center_x
        offset_y = target_y - center_y

        # Convert pixel offset to angle using atan (inverse of perspective projection)
        # This is the KEY formula that the original code got wrong
    ...
    def get_move(
        ...
        # Apply smoothing (simple division)
        if smoothing > 1.0:
        ...
        center_x, center_y = self.screen.center
        pixel_distance = sqrt((target_x - center_x)**2 + (target_y - center_y)**2)
